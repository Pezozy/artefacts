...
// ─── Calculator 9 — Portfolio Monitor ────────────────────────────────────────
...
const STATUS_COLORS = {
  Active: { dot: "bg-cyan-400", badge: "bg-cyan-900/40 text-cyan-300 border-cyan-700/40" },
  Exited: { dot: "bg-green-400", badge: "bg-green-900/40 text-green-300 border-green-700/40" },
  "Written Off": { dot: "bg-red-400", badge: "bg-red-900/40 text-red-300 border-red-700/40" },
};
...
const PIE_PALETTE = ["#0ea5e9", "#22c55e", "#f59e0b", "#8b5cf6", "#ef4444", "#ec4899", "#14b8a6", "#f97316", "#6366f1", "#84cc16"];
...
function computePortfolioMetrics(companies, currency) {
  ...
  const rows = companies.map((c) => {
    ...
    const holdingYears = invDate && invDate < today
    ...
    } else if (isFiniteNum(valuation) && isFiniteNum(check) && isFiniteNum(valuation)) {
      currentValue = null;
    }

    const moic = isFiniteNum(currentValue) && isFiniteNum(check) && check > 0
      ? currentValue / check : null;
    const irr = isFiniteNum(moic) && isFiniteNum(holdingYears) && holdingYears > 0
      ? (Math.pow(moic, 1 / holdingYears) - 1) * 100 : null;
    const tvpi = moic; // simplified: no recycling
  ...
  const totalValue = validRows.reduce((s, r) => s + (isFiniteNum(r.currentValue) ? r.currentValue : 0), 0);
  ...
  // Invested by sector for pie chart
  const bySector = {};
  validRows.forEach((r) => {
  ...
  const byStage = {};
  validRows.forEach((r) => {
    byStage[r.stage] = (byStage[r.stage] || 0) + r.check;
  });
  const stageData = Object.entries(byStage).map(([name, value]) => ({ name, value }));

  // Value by company for bar chart
  ...
  return { rows, totalInvested, totalValue, portfolioMoic, numActive, numExited, numWO, sectorData, stageData, companyBar };
}

function PortfolioMonitor({ currency }) {
  ...
  const addCompany = useCallback(() => {
    setInputs((prev) => {
      ...
      return { ...prev, companies: [...prev.companies, mkCompany(newId)] };
    });
  }, []);
  ...
  const metrics = useMemo(() => computePortfolioMetrics(inputs.companies, currency), [inputs.companies, currency]);
      ...
      >
        ...
        {metrics.totalInvested > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            {[
              ...
              { label: "Portfolio MOIC", value: isFiniteNum(metrics.portfolioMoic) ? metrics.portfolioMoic.toFixed(2) + "x" : "—" },
              { label: "Companies", value: `${metrics.numActive} active · ${metrics.numExited} exited · ${metrics.numWO} w/o` },
            ].map(({ label, value }) => (
        ...
        {metrics.sectorData.length > 0 && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
            {/* Invested by Sector pie */}
            <div className="bg-slate-800/40 border border-slate-700/40 rounded-xl p-4">
              <div className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-3">Invested by Sector</div>
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
                  <Pie data={metrics.sectorData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false} fontSize={10}>
                    {metrics.sectorData.map((_, i) => (
                      <Cell key={i} fill={PIE_PALETTE[i % PIE_PALETTE.length]} />
                    ))}
                  </Pie>
                  <Tooltip formatter={(v) => fmt(v, "currency", currency)} contentStyle={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 8, color: "#f1f5f9", fontSize: 12 }} />
                </PieChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="bg-slate-800/40 border border-slate-700/40 rounded-xl p-4">
              <div className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-3">Invested vs. Current Value</div>
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={metrics.companyBar} margin={{ top: 0, right: 8, left: 0, bottom: 30 }}>
                  <XAxis dataKey="name" tick={{ fontSize: 10, fill: "#94a3b8" }} angle={-30} textAnchor="end" interval={0} />
                  <YAxis tick={{ fontSize: 10, fill: "#94a3b8" }} tickFormatter={(v) => fmt(v, "currency", currency)} width={70} />
                  <Tooltip formatter={(v) => fmt(v, "currency", currency)} contentStyle={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 8, color: "#f1f5f9", fontSize: 12 }} />
                  <Bar dataKey="invested" name="Invested" fill="#334155" radius={[3, 3, 0, 0]} />
                  <Bar dataKey="currentValue" name="Current Value" fill="#0ea5e9" radius={[3, 3, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="space-y-2 mb-4">
          ...
          <div className="hidden lg:grid grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_1fr_auto] gap-2 px-3 pb-1">
            {["Company", "Sector", "Stage", "Status", "Invested", "Valuation", "MOIC", ""].map((h) => (
              <div key={h} className="text-[10px] uppercase tracking-widest text-slate-500">{h}</div>
            ))}
          </div>
          ...
          {inputs.companies.map((company, idx) => {
            ...
            return (
              <div key={company.id} className="bg-slate-800/50 border border-slate-700/40 rounded-xl overflow-hidden">
                {/* ── Collapsed row ── */}
                <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr_1fr_1fr_1fr_1fr_1fr_auto] gap-2 items-center p-3">
                  ...
                  <InputField
                    label=""
                    value={company.status === "Exited" ? company.exitProceeds : company.currentValuation}
                    onChange={(val) => updateCompany(company.id, company.status === "Exited" ? "exitProceeds" : "currentValuation", val)}
                    placeholder={company.status === "Exited" ? "Exit $" : "Valuation"}
                    prefix={currency === "USD" ? "$" : currency === "EUR" ? "€" : currency === "GBP" ? "£" : ""}
                    type="number"
                  />
                  {/* MOIC */}
                  <div className={`text-sm font-bold text-right lg:text-left ${moicColor(row?.moic)}`}>
                ...
                {isExpanded && (
                  <div className="border-t border-slate-700/40 bg-slate-900/30 p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {/* Investment date */}
                    <div>
                      <label className="block text-[10px] uppercase tracking-widest text-slate-500 mb-1">Investment Date</label>
                      <input
                        type="date"
                        value={company.investmentDate}
                        onChange={(e) => updateCompany(company.id, "investmentDate", e.target.value)}
                    ...
                    {/* Computed metrics */}
                    {(isFiniteNum(row?.moic) || isFiniteNum(row?.irr) || isFiniteNum(row?.currentValue)) && (
                      <div className="sm:col-span-2 lg:col-span-3 bg-slate-800/60 border border-slate-700/30 rounded-xl p-3 grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {[
                          { label: "Current Value", value: isFiniteNum(row?.currentValue) ? fmt(row.currentValue, "currency", currency) : "—" },
                          { label: "MOIC", value: isFiniteNum(row?.moic) ? row.moic.toFixed(2) + "x" : "—" },
                          { label: "IRR (est.)", value: isFiniteNum(row?.irr) ? fmtPct(row.irr) : "—" },
                          { label: "Holding (yrs)", value: isFiniteNum(row?.holdingYears) ? row.holdingYears.toFixed(1) + " yr" : "—" },
                        ].map(({ label, value }) => (
                          <div key={label}>
